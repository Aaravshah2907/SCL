...
// If every child of a group has been suppressed in this context, then hide the group container
document.addEventListener("DOMContentLoaded", function() {
    var listItems = document.querySelectorAll('.nav_toc li');

    listItems.forEach(function(li) {
        var anchor = li.querySelector('a[href="#"]');
        
        if (anchor && li.querySelector('ul') && li.querySelector('ul').children.length === 0) {
...
JST['dropdown_item'] = _.template(
    '<% _.each(prodNavList, function(prod) { %>' +
      '<li style="display: list-item;" class="dropdown-item"><a href="<%= docRoot %>/<%= getReleaseNotesPage(prod.helplocation) %>"><%= prod.displayname %></a></li>' +
    '<% }); %>'
);

function getReleaseNotesPage(page) {
    // Whether page="matlab" or page="matlab/index.html",
    // return matlab/release-notes.html
