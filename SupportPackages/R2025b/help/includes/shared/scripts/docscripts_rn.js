...
window.addEventListener('load', function () {
    ...
    const scrollContainer = document.getElementById('doc_center_content');
    const scrollSpy = new bootstrap.ScrollSpy(scrollContainer, {
        target: '#pnav',
        offset: getScrollTopAdjustment() + 1
    });
});
...
$(document).ready(function () {
    ...
    $("#content_container").delegate(".expandAllLink", "click", function (e) {
        ...
        if (link.data('allexpanded')) {
            doCollapse(link);
            setExpandAllLinkState(link, false, "expand_all");
        } else {
            doExpand(link);
            setExpandAllLinkState(link, true, "collapse_all");
        }
    });
...
function buildLeftNav(shortName, pathToDocRoot, data) {
  ...
  var navElt = $("#rn-nav-tabs > ul.nav_toc");
  $(data).each(function(idx,product) {
    if (!products_without_rn.includes(product.shortname)) {
  ...
  var activeSibling = $("#rn-nav-tabs > ul.nav_toc > li.active");
  if (activeSibling.length > 0) {
    var leftNavScrollTo = activeSibling.get(0).offsetTop;
    var categoriesTop = $("#rn-nav-tabs").get(0).offsetTop;
    $("#rn-nav-tabs").scrollTop(leftNavScrollTo-categoriesTop);
  }
}
...
function setExpandAllLinkState(link, state, linkHtmlText) {
    link.data('allexpanded', state);
    link.html(getLocalizedString(linkHtmlText));
}

function applySearchHighlight() {
    if ($.fn.highlight) {
        var highlighterCSSClass = ['highlight_01', 'highlight_02' , 'highlight_03', 'highlight_04', 'highlight_05'];
        var searchHighlightTerm = $.getParameterByName('searchHighlight').replace(/&#34;/g,'"');
...
function doExpand(elt, noAnimation) {
    ...
    expandable.find('.collapse').slideDown(function () {
        ...
        triggerContentResize();
        checkExpandAllLinkState(elt);
        checkExpandAllPageLinkState(elt);
    });
}

function doCollapse(elt, noAnimation) {
    var expandable = findExpandableContent(elt);
    //Before collapsing, check if the collapse child has any expandableContent descendants.
    //If it does, those divs need to be collapsed and not the parent ('.collapse' child div).
    var collapsedChild = expandable.children('.collapse');
    if (collapsedChild.find('.expandableContent').length > 0) {
        expandable = expandable.children('.collapse').find('.expandableContent');
    }
...
function checkExpandAllPageLinkState(elt) {
    // Continue only if the event is on first '.expand' div in every release section
    if(elt.parents('.expandableContent').length != 2) {
        return;
    }
    var expandableParent = elt.parents('.expandableContent:last');
    var expandDivs = expandableParent.find('.expand');
    ...
    const expandAllPage = $('#expandAllPage');
    if(allChildrenCollapsed) {
        setExpandAllLinkState(expandAllPage, false, "expand_all_in_page");  
    } else if(allChildrenExpanded) {
        setExpandAllLinkState(expandAllPage, true, "collapse_all_in_page");
    }
}
...
function checkExpandAllLinkState(elt) {
    ...
    if (elt.hasClass('expanded')) {
        ...
        if (allChildrenExpanded) {
            setExpandAllLinkState(expandAllLink, true, "collapse_all");
        }
    } else {
        var allChildrenCollapsed = true;
        expandableChildren.each(function () {
        ...
        if (allChildrenCollapsed) {
            setExpandAllLinkState(expandAllLink, false, "expand_all");
        }
    }
}
...
function getScrollTopAdjustment() {
    ...
    if (typeof getDocCenterType === 'function') {
        let docCenterType = getDocCenterType();
        if(docCenterType === 'product') {
            scrollTop = $('.sticky_header_container').height() + $('.section_header').height() + 10; // approximately 106
        } else {
            scrollTop = $('.topNavigation').height() + $('.localnav').height() + 10; // approximately 101.333
        }
    }
