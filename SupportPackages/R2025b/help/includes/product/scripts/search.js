...
JST['pagination'] = _.template(
    '<div class="float-end add_font_color_general">' +
        '<a href="<%= getPageUrl(footer.selectedpage - 1) %>"' +
        '<% if(footer.selectedpage == 1) { %>' +
          ' class="inactive link_disabled" tabindex="-1"' +
        '<% } %>' +
          '><span class="icon-arrow-open-left icon_32 float-start"></span>' +
        '</a>' +

        '<p class="float-start add_margin_0" style="line-height:36px"><%= footer.summarytext %></p>' +

        '<a href="<%= getPageUrl(footer.selectedpage + 1) %>"' +
        '<% if(footer.selectedpage == footer.numpages) { %>' +
          ' class="inactive link_disabled" tabindex="-1"' +
        '<% } %>' +
        '><span class="icon-arrow-open-right icon_32 float-end"></span>' +
        '</a>' +
    '</div>'
);
