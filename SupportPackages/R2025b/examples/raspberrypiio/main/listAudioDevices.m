...
if numel(cards) > 1
    ...
    if strcmp(type,'playback')
        fileName = fullfile(raspi.internal.getRaspiRoot, 'raspi_examples', 'listAudioDevices_test.wav');
        hw.putFile(fileName);
    end
    ...
    for k = 1:numel(pcm)
        if ~isempty(regexp(pcm(k).Description,type,'match'))
            ...
            ret(end+1).Name = strrep(cards{cardNo+1},' ',''); %#ok<AGROW>
            ...
            if contains(hwParams,'Device or resource busy')
                deviceName = strrep(cards{cardNo+1},' ','');
                deviceNameFormatted = regexprep(deviceName,'[\n\r]+',' ');
                warning(message('raspi:utils:listAudioResourceBusy',deviceNameFormatted));
                ret(end).Channels = {};
                ret(end).BitDepth = {};
