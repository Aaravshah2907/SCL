...
% manipulating files using installed MATLAB.
...
%% Prerequisites
%
% * If you are new to MATLAB, refer to <docid:matlab_doccenter#bth9dea-1
% Get Started with MATLAB> documentation
% and <https://www.mathworks.com/videos/getting-started-with-matlab-101684.html Getting Started with MATLAB> video.
%
...
% with the support package. To update the Raspberry Pi firmware, follow the steps as described in <docid:mlsupportpkg#bt7xwov-2.mw_ce00da2f-bde9-4f39-a69a-a9aaf014dbc0 Hardware Setup>.
...
% through TCP/IP. 
%
% *Note*: If you encounter errors after running the above command, try using additional arguments (as listed in <docid:mlsupportpkg#buno_rn>) or refer to <docid:mlsupportpkg#bt7xwov-7>.
% 
...
% The user LED is, by default, configured to change its state based on the
% SD card activity. However, you can reconfigure the LED to change its
% state based on the requirement of your application. To manually control the
% LED, set the |system| command to |none|:  
%  
%   system(rpi, 'echo "none" | sudo tee /sys/class/leds/led0/trigger');
%
% The system commands with 0 and 1 are equivalent to the writeLED method with arguments 0 and
% 1 for the LED state.
%
%   system(rpi, 'echo 0 | sudo tee /sys/class/leds/led0/brightness');
%   system(rpi, 'echo 1 | sudo tee /sys/class/leds/led0/brightness');
%  
% You can set the LED back to its original state of tracking the SD card
% activity by executing this command:
%  
...
% <https://www.raspberrypi.org/documentation/remote-access/ Remote Access>. 
...
%
%% See Also
% Video series: <https://www.mathworks.com/videos/series/raspberry-pi-support-from-matlab.html Raspberry Pi Support for MATLAB>