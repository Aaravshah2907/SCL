...
% enable line. During SPI communication, the controller selects only
% one peripheral device at a time. Any other peripherals on the bus that has not been
% activated using its chip-select line must disregard the input clock and
% SDO signals. It also must not drive the SDI line.
%%
% <<../spi_diagram.png>>
%%
% SPI communications are full duplex. When the controller sends a bit on the
% SDO line, the peripheral reads the bit and at the same time sends a bit on
% the SDI line that is in turn read by the controller. Therefore, to read from
...
% support up to two SPI peripheral devices. Each SPI channel has a corresponding
